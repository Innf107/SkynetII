 ...
 */
const xcss = (folders : String[], options : XCssOptions = {}) => (req : express.Request, res : express.Response, next : express.NextFunction) => {
    ...
    for(const f of folders) {
        ...
        try{
            ...
            if(ops.verbose)
                console.log(`${fullDir} does not exist. This is not neccessarily an error!`)
        }
    }
    if(ops.verbose)
...
const parseCSS = (css : string, mobile : boolean) => {
    const rgxG = /\s*([A-Za-z-]+)\s*:\s*([A-Za-z0-9%_ -]+)\s*\+\s*([A-Za-z0-9%_ -]+);?/g
    const rgx = /\s*([A-Za-z-]+)\s*:\s*([A-Za-z0-9%_ -]+)\s*\+\s*([A-Za-z0-9%_ -]+);?/
